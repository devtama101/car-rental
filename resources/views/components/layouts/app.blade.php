    ...
    <body class="bg-gray-50 text-gray-900 min-h-screen antialiased">
        <header class="bg-white border-b border-gray-200">
            <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
                <a href="/" class="text-xl font-bold text-green-700">{{ __('CarRental') }}</a>
                <nav class="flex items-center gap-4 text-sm">
                    <div class="flex items-center gap-1 text-xs border-r border-gray-200 pr-4 mr-1">
                        <a href="{{ route('language', 'en') }}" class="{{ app()->getLocale() === 'en' ? 'font-bold text-green-700' : 'text-gray-400 hover:text-gray-600' }} no-underline">EN</a>
                        <span class="text-gray-300">|</span>
                        <a href="{{ route('language', 'id') }}" class="{{ app()->getLocale() === 'id' ? 'font-bold text-green-700' : 'text-gray-400 hover:text-gray-600' }} no-underline">ID</a>
                    </div>
                    @auth
